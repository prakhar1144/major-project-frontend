...
const NearbyStation = () => {
  ...
  const getNearbyData = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        let tempRows = response.data.values.map((item, index) => {
          console.log(typeof index);
          if (index > 0) {
            const obj = {
              id: index,
              Name: item[0],
              Connector_Type: item[3],
              Status: item[4],
              Rate: parseInt(item[5], 10),
              Free_Connector_Count: parseInt(item[6], 10)
            };
  ...
  return (
    <>
      <iframe
        title="nearby_charging_stations"
        src="https://www.google.com/maps/d/u/0/embed?mid=1y4_hgDCpK0rKgTD9VFtdoqHV3wol_F4&ehbc=2E312F"
        width="100%"
        height="480"
      />
      <br />
